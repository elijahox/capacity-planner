...
app.post('/api/seed', async (req, res) => {
  ...
  try {
    ...
    if (!currentState) {
      return res.status(400).json({ ok: false, error: 'No state in database to snapshot' });
    }
    const newSeedContent =
      '// ================================================================\n' +
      '// DEFAULT STATE — used by db.js to seed an empty database\n' +
      '// ================================================================\n' +
      '// Auto-generated by Save as Seed on ' + new Date().toISOString().slice(0, 10) + '\n' +
      '// Do not edit manually — use the Save as Seed button in the app.\n\n' +
      'module.exports = ' + JSON.stringify(currentState, null, 2) + ';\n';
    fs.writeFileSync(path.join(__dirname, 'seed.js'), newSeedContent);
    console.log('🌱 Seed file updated from live database state');
    res.json({ ok: true, message: 'Seed updated' });
  } catch (e) {
    console.error('POST /api/seed error:', e.message);
    res.status(500).json({ ok: false, error: 'Failed to write seed file' });
  }
});
